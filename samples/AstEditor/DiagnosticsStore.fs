...
module DiagnosticsStore =
    ...
    let private table =
        ConditionalWeakTable<TextEditor, Intellisense.Diagnostic[] ref>()
    ...
    /// Drop an editor's diagnostics — called on every edit, because their line/column ranges
    /// were computed against the previous text and acting on them (hover, quick fixes) could
    /// target the wrong span. The next debounced check repopulates them.
    let clear(editor: TextEditor) = set editor [||]

    /// Is the (1-based line, 0-based column) position inside this diagnostic's range?
    /// FCS end columns are exclusive (one past the last character); zero-width ranges
    /// (e.g. unexpected EOF) still match at their anchor column.
    let covers (line: int) (col: int) (d: Intellisense.Diagnostic) =
        let afterStart = line > d.StartLine || (line = d.StartLine && col >= d.StartColumn)

        let beforeEnd =
            line < d.EndLine
            || (line = d.EndLine
                && (col < d.EndColumn
                    || (d.StartLine = d.EndLine && d.StartColumn = d.EndColumn && col = d.EndColumn)))
