...
module Match =
    ...
    let WidgetKey = Widgets.register "Match" (fun widget ->
        let matchExpr = Helpers.getNodeFromWidget<Expr> widget MatchExpr
        let matchClauses = Helpers.getNodesFromWidgetCollection<MatchClauseNode> widget MatchClauses
...
type MatchYieldExtensions =
    [<Extension>]
    static member inline Yield(_: CollectionBuilder<Expr, MatchClauseNode>, x: WidgetBuilder<MatchClauseNode>) : Content =
